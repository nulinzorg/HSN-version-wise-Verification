...
# Configurable so the exact same module can write output either next to
# itself (local/Flask use) or into a specific folder like docs/data (when
# run from a GitHub Action, so GitHub Pages can serve the results as
# static files). Set HSN_DATA_DIR to override; defaults to the old
# behavior of writing next to this file.
...
def _save_history(history: list):
    # Sorted most-recent-first BEFORE writing to disk (not just when
    # read back via load_history()) - this file is served directly as a
    # static asset by GitHub Pages, so there's no server available to
    # sort it on request; it has to already be in the right order.
    sorted_history = sorted(history, key=lambda h: h["run_id"], reverse=True)
    HISTORY_FILE.write_text(json.dumps(sorted_history, indent=2, ensure_ascii=False), encoding="utf-8")
